...
export const Sidebar = () => {
  ...
  return (
    <>
      <aside className={`${visible ? '-translate-x-0' : 'translate-x-full'} transition-transform duration-100 ease-in-out fixed right-0 inset-y-0 z-50 bg-white ring-1 ring-black/5 w-full max-w-sm p-5`}>
        ...
        <form>
          ...
          <div>
            ...
            <textarea name="description" id="description" className="p-4 w-full border border-black/20 rounded-xl" />
          </div>
          <button className="block bg-black text-white p-3 mt-5 w-full rounded-full">Create collection</button>
        </form>
      </aside>
      <div className={`${visible ? 'block' : 'hidden'} fixed inset-0 bg-white/50 backdrop-blur z-40`} />
    </>
  );
};
...
const FormInput: React.FC<FormInputProps> = ({ name, label, className }) => (
  <div className={className}>
    ...
    <input type="text" name={name} id={name} className="p-2 rounded-xl border border-black/20 w-full mb-5 mt-2" />
  </div>
)
